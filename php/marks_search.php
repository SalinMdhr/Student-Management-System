                        ...
                        echo "<table>";
                            ...
                            if(isset($_POST['submit'])){
                                ...
                                $search_Query = "SELECT
                                    ...
                                    LEFT JOIN board b ON s.ID = b.ID WHERE CONCAT(F_name,' ',L_name) LIKE '%$filterValues%';
                                ";
                                $search_Query_Exe = mysqli_query($conn,$search_Query);

                                if(mysqli_num_rows($search_Query_Exe) > 0){
                                    $flag = 0;
                                    while($row = mysqli_fetch_assoc($search_Query_Exe)){
                                        $id = $row['ID'];
                                        $f_name = $row['F_name'];
                                        $l_name = $row['L_name'];
                                        ...
                                        echo "<tr>";
                                            ...
                                            if (empty($row['bd_se']) && empty($row['bd_dbms']) && empty($row['bd_os']) && empty($row['bd_nm']) && empty($row['bd_sl'])) {
                                                if($flag == 1){
                                                    echo "<td colspan=5><a href='add_Marks.php?ID=$id&term=1'><input type='button' value='Add Marks'></a></td></a></td>";
                                                } else if($flag == 2){
                                                    echo "<td colspan=5><a href='add_Marks.php?ID=$id&term=2'><input type='button' value='Add Marks'></a></td>";
                                                } else if($flag == 3){
                                                    echo "<td colspan=5><a href='add_Marks.php?ID=$id&term=3'><input type='button' value='Add Marks'></a></td>";
                                    ...
                                    echo "<tr>
                                            <td colspan='34'>No Record Found</td>
                                        </tr>";
                                }
                            }
